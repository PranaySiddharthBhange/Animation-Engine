...
const options = {
    ...
    accessToken: 'eyJhbGciOiJSUzI1NiIsImtpZCI6IlhrUFpfSmhoXzlTYzNZS01oRERBZFBWeFowOF9SUzI1NiIsInBpLmF0bSI6ImFzc2MifQ..Gynsa89R0zu_zY-r69WKfD78ewTFMnDUywDoKCbxB-D3pc8b9ghAy7QeqNHP5IkPIOffZ-GRadVdikKwr-oZR6bzrR1_RrW_fQ6UPGQt4vGQpRy9CRLcRZfen2RQiICP3dvYUS2pz43uteytg1BvRyY9u2MoXAEJLuiaKUtjU0unwDYPPVoWUL7mKXdDYecJ13oNmTRBZNFf4WbJXK7FgwNE2p_AYD2aG39-JrTLQJmVl4BXoaarpFf7-pLmETRjAuiV02sfeYeaTP2Wmv2iseNvb2eiDblhK4tR0XojgO6ewY0fIhKPzbR4u_T8h0WiuQPX-Gr-6wVGOYnGZkHzBg'
};
const documentId = 'urn:dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6MTNqdW5lXzIwMjUvQXNzZW1ibHkxLmlhbQ';
...
// ✅ Reframed complete and correct Gemini animation generator function
async function getGeminiAnimationCommands() {
    ...
    try {
        const [hierarchyRes, propertiesRes, jointsRes, disassemblyRes] = await Promise.all([
            fetch('responses/09_object_hierarchy.json'),
            fetch('responses/10_properties_all_objects.json'),
            fetch('responses/11_joints_constraints.json'),
            fetch('responses/13_disassembly_sequence.json')
        ]);

        if (!hierarchyRes.ok || !propertiesRes.ok || !disassemblyRes.ok) {
            throw new Error("Failed to fetch one or more required response files.");
        }

        const hierarchyData = await hierarchyRes.json();
        const propertiesData = await propertiesRes.json();
        const disassemblyData = await disassemblyRes.json();

        const disassemblySteps = disassemblyData.disassembly_steps;
...
Generate a disassembly and reassembly animation sequence using rotate, translate, and scale operations.

Use this disassembly sequence:
${JSON.stringify(disassemblySteps)}
...
Each command must be a JSON object like:
{
  "fragmentId": <number>,
  ...
  "params": {
    ...
    "x": <number>, "y": <number>, "z": <number>  // for translate
  },
  "duration": <number> // in seconds
}

Guidelines:
1. Follow the disassembly sequence order.
2. Outer parts first, inner parts later.
3. After disassembly, reverse for reassembly.
4. 2-3 actions per part; total 30-50 steps.
5. Use accurate fragmentId only — do not make up IDs.
...
Return only the JSON array.`.trim();
        ...
        console.log("Gemini parsed animation commands:", commands);
        return commands;

    } catch (e) {
        logToConsole(`Gemini animation error: ${e.message}`, true);
        return [];
    } finally {
