...
const UploadPage = memo(({ onProcessingStart }) => {
...
const ProcessingPage = memo(({ sessionId, onComplete, onError }) => {
...
// Memoized CountdownTimer for performance
const CountdownTimer = memo(({ expirationTime }) => {
  const [timeLeft, setTimeLeft] = useState(expirationTime - Date.now());

  useEffect(() => {
    const interval = setInterval(() => {
      setTimeLeft(Math.max(expirationTime - Date.now(), 0));
    }, 1000);
...
const ResultsPage = memo(({ resultData, onStartNew, onTokenExpired }) => {
  ...
  const handleTokenExpired = useCallback((errorMessage) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (Date.now() >= expirationTime) setSessionValid(false);
    }, 1000);
    return () => clearInterval(interval);
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-800 mb-4">
            {tokenError ? 'Access Token Error' : 'Session Expired'}
          </h1>
          <p className="text-gray-600 mb-6">
            {tokenError || 'Your viewing session has ended. Please process a new file.'}
          </p>
          <button
            onClick={onStartNew}
...
const App = () => {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem('viewerSession');
    if (savedData) {
      const parsed = JSON.parse(savedData);
      const isValid = Date.now() < (parsed.timestamp + 23 * 60 * 60 * 1000);
      if (isValid) {
  ...
  const handleProcessingStart = useCallback((newSessionId) => {
  ...
  const handleProcessingComplete = useCallback((data) => {
    ...
    localStorage.setItem('viewerSession', JSON.stringify(fullData));
