...
const ProcessingPage = ({ sessionId }) => {
  const [message, setMessage] = useState('Starting...');
  const [progress, setProgress] = useState(0);
  const { handleProcessingComplete, handleProcessingError } = useAppContext();
    ...
    const checkStatus = async () => {
      try {
        ...
        if (data.status === 'completed') {
          ...
          clearInterval(intervalId);
          handleProcessingError('Processing failed');
        }
      } catch (err) {
        console.error('Status check error:', err);
        handleProcessingError('Failed to check status');
      }
    };
  ...
  }, [sessionId, handleProcessingComplete, handleProcessingError]);
