...
def create_activity(token):
    ...
    command_line = (
        "$(engine.path)\\InventorCoreConsole.exe /al \"$(appbundles[joints_constraints_appbundle].path)\" /c ExtractJointsAndConstraints"
    )
...
def submit_workitem(access_token):
    ...
    payload = {
        ...
        "arguments": {
            "outputJson": {  
                "url": f"urn:adsk.objects:os.object:{BUCKET_KEY}/output.json",
                "verb": "put",
                "localName": "output.json",
                "headers": {
                    "Authorization": f"Bearer {access_token}"
            ...
            "errorOutput": {  
                "url": f"urn:adsk.objects:os.object:{BUCKET_KEY}/error.log",
                "verb": "put",
                "localName": "error.log",
                "headers": {
                    "Authorization": f"Bearer {access_token}"
...
def check_workitem_status(access_token, workitem_id):
    ...
    while True:
        if time.time() - start_time > timeout:
            ...
            break
        response = requests.get(url, headers=headers)
        save_response_to_file("13_workitem_status", response.json())
...
if __name__ == "__main__":
    ...
    if workitem_result.get("status") == "success":
        url = get_signed_download_url(token, "output.json")
        if url:
            download_file(url, "output.json")
            print("✅ Successfully downloaded results (output.json)")
        else:
            print("❌ output.json not found.")
    else:
        url = get_signed_download_url(token, "error.log")
        if url:
            download_file(url, "error.log")
            print("❌ Error logs downloaded (error.log)")
        else:
